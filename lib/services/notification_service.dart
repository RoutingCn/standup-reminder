...
class NotificationService {
  ...
  static Future<void> init() async {
    ...
    await _plugin.resolvePlatformSpecificImplementation<AndroidFlutterLocalNotificationsPlugin>()
        ?.createNotificationChannel(const AndroidNotificationChannel(_channelId, _channelName, description: _channelDesc, importance: Importance.high, playSound: true, enableVibration: true));
  }

  static Future<void> scheduleNextReminder(int intervalMinutes) async {
    await _plugin.cancel(_notificationId);
    final now = DateTime.now();
    final scheduled = tz.TZDateTime.from(now.add(Duration(minutes: intervalMinutes)), tz.local);
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('next_reminder_at', scheduled.toIso8601String());
    final hints = ['颈部拉伸','肩部放松','深蹲几个','深呼吸','活动手腕','站起来走走'];
    await _plugin.zonedSchedule(
      _notificationId, '⏰ 该起身活动一下了！',
      '已经坐了 $intervalMinutes 分钟，来做个 ${hints[DateTime.now().millisecond % hints.length]} 吧',
      scheduled,
      const NotificationDetails(
        android: AndroidNotificationDetails('standup_reminder', '起身提醒', channelDescription: '久坐定时提醒', importance: Importance.high, priority: Priority.high, icon: '@mipmap/ic_launcher'),
